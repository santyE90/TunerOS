...
inline constexpr std::uint16_t kDmeCombustionObservationFrameId = 0x503;
...
inline constexpr std::uint8_t kDmeCombustionObservationPayloadLength = 3;
...
inline constexpr std::uint64_t kDmeCombustionObservationPeriodMicroseconds = 20'000;
...
[[nodiscard]] canbus::CanFrame make_dme_combustion_observation_frame(
    const simulator::VehicleState& state);
